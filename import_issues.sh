...
process_row() {
  ...
  if [ -z "$issue_url" ]; then
    echo "Failed to create or find issue: '$issue_title'" >> "$ERROR_LOG_FILE"
    return # Continue to next row even on failure
  fi
  ...
  if [ -z "$item_id" ]; then
    echo "Failed to add issue '$issue_title' to project." >> "$ERROR_LOG_FILE"
  ...
  for field_name in "${!project_fields[@]}"; do
      ...
      gh project item-edit --id "$item_id" --field-name "$field_name" --text "$field_value" || echo "Failed to update field '$field_name' for issue '$issue_title'" >> "$ERROR_LOG_FILE"
  done
}

# --- Execution ---
# Use a while-read loop to process the file line by line.
# This is much safer than xargs for complex, multi-line, or special character input.
# It reads each line from the tsv file (skipping the header) into the 'line' variable.
tail -n +2 "$DATA_FILE" | while IFS= read -r line; do
    # Call the process_row function for each line.
    # We add error handling here to log failures but continue the script.
    process_row "$line" || {
        echo "---" >> "$ERROR_LOG_FILE"
...
if [ -f "$ERROR_LOG_FILE" ]; then
  ...
  # We no longer exit with 1, so the workflow commit step can run.
fi
