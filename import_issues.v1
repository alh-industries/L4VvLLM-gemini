...
process_row() {
 ...
 # done
  ...
  if [ -n "$existing_issue_url" ]; then
    ...
    # Edit existing issue (body and labels)
    gh issue edit "$issue_url" --body "$issue_body" $(for label in "${issue_labels[@]}"; do echo "--add-label" "'$label'"; done)
  else
  ...
  for field_name in "${!project_fields[@]}"; do
      ...
      gh project item-edit --id "$item_id" --set "$field_name"="$field_value"
  done
}
