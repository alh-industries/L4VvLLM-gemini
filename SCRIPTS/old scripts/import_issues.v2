...
process_row() {
  ...
  # 1. Create all labels with a random color. `|| true` ignores errors if the label already exists.
  ...
  # Prepare labels for gh command (only used for new issue creation)
  labels_arg=$(printf ",%s" "${issue_labels[@]}")
  labels_arg=${labels_arg:1}
  ...
  if [ -n "$existing_issue_url" ]; then
    ...
    for label in "${issue_labels[@]}"; do
        gh issue edit "$issue_url" --add-label "$label" || true # Ignore error if label is already on issue
    done
  else
    echo "No existing issue found. Creating new issue."
    # Create new issue and capture its URL
    issue_url=$(gh issue create --title "$issue_title" --body "$issue_body" --label "$labels_arg" --json url -q '.url')
  fi
...
# If any `process_row` call fails, `log_error` is called with the failed row.
